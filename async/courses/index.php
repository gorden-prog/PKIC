    <section class="courses">
        <div class="content">
            <div class="course">
                <div class="course__descript">
                    <img src="/assets/img/courses/ai.png" alt="Искусственный интеллект">
                    <div class="course__descript__text">
                        <h2>Искусственный интеллект: с нуля до специалиста</h2>
                        <p>Смотрите, как работает чужой код, копируйте его, называйте нейросетью и кладите в портфолио. Курс подходит для тех, кто хочет писать "ML-инженер" в резюме, не зная, что это значит.</p>
                    </div>
                </div>
                <div class="course__buy">
                    <div class="course__buy__wrap">
                        <div class="course__buy__wrap__modules">
                            <ul>
                                <li>Python для ИИ</li>
                                <li>Машинное обучение</li>
                                <li>Нейронные сети</li>
                                <li>Финальный проект</li>
                            </ul>
                        </div>
                        <div class="course__buy__wrap__price">
                            <ul>
                                <li>Длительность: 6 месяцев</li>
                                <li>Стоимость: 49 900 ₽</li>
                                <li>В рассрочку: 4 158 ₽/мес</li>
                            </ul>
                        </div>
                    </div>
                    <a onclick="openPage('main', true);">ПЕРЕЙТИ К КУРСУ</a>
                </div>
            </div>

            <div class="course">
                <div class="course__descript">
                    <img src="/assets/img/courses/business.png" alt="Бизнес-аналитика">
                    <div class="course__descript__text">
                        <h2>Бизнес-аналитика: от данных к решениям</h2>
                        <p>Откройте Excel, нарисуйте красивые стрелочки в BPMN 2.0 и сделайте вид, что анализируете данные. После курса вы сможете убедить любого руководителя, что его проблемы — в неправильных графиках.</p>
                    </div>
                </div>
                <div class="course__buy">
                    <div class="course__buy__wrap">
                        <div class="course__buy__wrap__modules">
                            <ul>
                                <li>Excel Pro</li>
                                <li>SQL и базы данных</li>
                                <li>Power BI</li>
                                <li>Python для аналитика</li>
                                <li>Статистика и прогнозирование</li>
                                <li>Финальный проект</li>
                            </ul>
                        </div>
                        <div class="course__buy__wrap__price">
                            <ul>
                                <li>Длительность: 4 месяца</li>
                                <li>Стоимость: 39 900 ₽</li>
                                <li>В рассрочку: 3 325 ₽/мес</li>
                            </ul>
                        </div>
                    </div>
                    <a onclick="openPage('main', true);">ПЕРЕЙТИ К КУРСУ</a>
                </div>
            </div>
        </div>
    </section>

    <section class="pricing">
        <div class="content">
            <h2>Сравнение курсов</h2>
            <table class="pricing__table">
                <thead>
                    <tr>
                        <th>Курс</th>
                        <th>Длительность</th>
                        <th>Стоимость</th>
                        <th>В рассрочку</th>
                        <th>Формат</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Искусственный интеллект: с нуля до специалиста</td>
                        <td>6 месяцев</td>
                        <td>49 900 ₽</td>
                        <td>4 158 ₽/мес</td>
                        <td>Онлайн · Живые вебинары</td>
                    </tr>
                    <tr>
                        <td>Бизнес-аналитика: от данных к решениям</td>
                        <td>4 месяца</td>
                        <td>39 900 ₽</td>
                        <td>3 325 ₽/мес</td>
                        <td>Онлайн · Живые вебинары</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </section>
